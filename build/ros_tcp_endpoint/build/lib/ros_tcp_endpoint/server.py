...
class TcpServer(Node):
    ...
    def listen_loop(self):
        ...
        # Be resilient to stale processes holding the port.
        # If bind fails, retry for a short window rather than crashing the listen thread.
        bind_deadline = time.time() + 10.0
        while True:
            try:
                ...
                if time.time() >= bind_deadline:
                    self.logerr(f"Failed to bind {self.tcp_ip}:{self.tcp_port} after retries: {e}")
                    return
                self.logwarn(f"Bind failed ({e}); retrying...")
                time.sleep(0.5)
        ...
        while True:
            ...
            try:
                ...
                self.logerr(f"ros_tcp_endpoint.TcpServer: socket error: {e}")
                time.sleep(0.2)
...
class SysCommands:
    ...
    def subscribe(self, topic, message_name):
        ...
        # If the subscriber already exists for this topic, reuse it to avoid
        # re-creating a Node with the same name (which can trigger rosout warnings).
        existing = self.tcp_server.subscribers_table.get(topic)
        if existing is not None:
            self.tcp_server.loginfo(
                "RegisterSubscriber({}, {}) OK (already registered)".format(topic, message_class)
            )
            return
    ...
    def publish(self, topic, message_name, queue_size=10, latch=False):
        ...
        # Same idea as subscribe(): reuse an existing publisher for the topic to avoid
        # repeated Node construction/destruction and rosout "already registered" warnings.
        existing = self.tcp_server.publishers_table.get(topic)
        if existing is not None:
            self.tcp_server.loginfo(
                "RegisterPublisher({}, {}) OK (already registered)".format(topic, message_class)
            )
            return
