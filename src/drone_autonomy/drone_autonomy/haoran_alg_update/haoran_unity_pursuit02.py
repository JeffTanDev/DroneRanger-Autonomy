...
class HaoranUnityPursuit(Node):
    def __init__(self):
        ...
        self.declare_parameter("vfov_deg", 60.0)               # 相机垂直视场角（度），用于 2D 扇区
        self.declare_parameter("num_sectors", 9)              # 水平扇区数
        self.declare_parameter("num_sectors_vertical", 5)     # 垂直扇区数（2D 时有效）
        self.declare_parameter("use_2d_sectors", True)       # True=前方全角度(水平+垂直)，False=仅水平
        ...
        # 上下避障：根据深度图上下区域调整高度
        self.declare_parameter("vertical_avoid_enable", True)
        self.declare_parameter("vertical_near_m", 1.5)           # 上下方小于此距离视为有障碍
        self.declare_parameter("vertical_offset_max_m", 2.0)     # 相对目标高度最大上下偏移（米）
        self.declare_parameter("vertical_roi_width_ratio", 0.5)  # 竖直条带占图像宽度比例（取中间一段）
        # 比视野大的障碍物：前方整体很近时停+爬升，扇区膨胀避免从边缘钻过
        self.declare_parameter("large_obstacle_threshold_m", 1.2)  # 前方全局最近距离低于此视为“大墙”，停并爬升
        self.declare_parameter("large_obstacle_climb_m", 0.5)       # 大障碍时每周期指令爬升高度（米）
        self.declare_parameter("sector_inflation_cells", 1)         # 近障扇区向邻格膨胀格数（0=不膨胀）
    ...
    def _depth_sectors_2d(self):
        """
        将深度图划分为 水平×垂直 的 2D 扇区网格，覆盖前方所有角度（水平+垂直）。
        近障扇区会向邻格膨胀，避免大物体只占部分视野时从“边缘”钻过。
        返回: list of (angle_h, angle_v, d_min)，angle_h 为水平角（弧度），angle_v 为俯仰角（弧度，上正下负），
              d_min 为该格最近有效距离（米），无数据为 None。
        """
        if self.depth_image is None:
            return []
        now = time.time()
        ...
        h, w = self.depth_image.shape
        num_h = int(self.get_parameter("num_sectors").value)
        num_v = int(self.get_parameter("num_sectors_vertical").value)
        num_h = max(2, num_h)
        num_v = max(2, num_v)

        hfov_deg = float(self.get_parameter("hfov_deg").value)
        vfov_deg = float(self.get_parameter("vfov_deg").value)
        hfov_rad = math.radians(hfov_deg)
        ...
        inflate = int(self.get_parameter("sector_inflation_cells").value)

        cell_w = w / num_h
        cell_h = h / num_v
        # 先建 2D 网格（用 inf 表示无数据，便于膨胀时 min）
        grid = np.full((num_v, num_h), np.inf, dtype=np.float64)
        for j in range(num_v):
            ...
            for i in range(num_h):
                ...
                roi = roi[(roi >= min_valid) & (roi <= max_valid)]
                if roi.size > 0:
                    grid[j, i] = float(np.percentile(roi, depth_percentile))

        # 近障膨胀：若某格 d < min_clear，邻格有效距离收紧，避免大物体边缘被当成“可钻”
        if inflate > 0:
            grid2 = grid.copy()
            for j in range(num_v):
                for i in range(num_h):
                    d = grid[j, i]
                    if d < min_clear and np.isfinite(d):
                        for dj in range(-inflate, inflate + 1):
                            for di in range(-inflate, inflate + 1):
                                nj, ni = j + dj, i + di
                                if 0 <= nj < num_v and 0 <= ni < num_h and np.isfinite(grid[nj, ni]):
                                    grid2[nj, ni] = min(grid2[nj, ni], d * 1.2)
            grid = grid2

        # 转成 list of (angle_h, angle_v, d_min)，inf 转 None
        out = []
        for j in range(num_v):
            y0 = int(j * cell_h)
            y1 = int((j + 1) * cell_h)
            ...
            if y1 <= y0:
                continue
            center_row = (y0 + y1) * 0.5
            angle_v = vfov_rad * 0.5 - (center_row / h) * vfov_rad
            for i in range(num_h):
                center_col = (i + 0.5) * cell_w
                rel = center_col / w - 0.5
                angle_h = rel * hfov_rad
                d = grid[j, i]
                out.append((angle_h, angle_v, None if not np.isfinite(d) else float(d)))
        return out

    def _vfh_choose_direction_2d(self, goal_yaw: float, goal_elevation: float):
        """
        2D VFH：在水平+垂直所有扇区中选一个“安全且朝向目标”的方向。
        goal_elevation：目标相对水平面的俯仰角（弧度，上正下负），如 atan2(-(goalD - pz), goal_dist_xy)。
        返回: (best_yaw, best_dist, best_elevation)，无可行扇区时为 (None, None, 0.0)。
        """
        cells = self._depth_sectors_2d()
        if not cells:
        ...
        for angle_h, angle_v, d_min in cells:
            if d_min is None:
                continue
            if d_min < stop_dist:
                continue
            cand_yaw = radians_wrap(self.heading + angle_h)
            yaw_diff = abs(radians_wrap(cand_yaw - goal_yaw))
            elev_diff = abs(angle_v - goal_elevation)
            if d_min >= min_clear:
                dist_cost = 0.0
            else:
                dist_cost = (min_clear - d_min) / max(min_clear, 1e-3)
            score = yaw_diff + 0.5 * elev_diff + 2.0 * dist_cost
            if best_score is None or score < best_score:
                best_score = score
                best_yaw = cand_yaw
                best_dist = d_min
                best_elevation = angle_v

        return best_yaw, best_dist, best_elevation
    ...
    def _depth_vertical_bands(self):
        """
        将深度图在竖直方向分为上、下两段（图像上方=机体上方/天花板，下方=机体下方/地面），
        返回各段最近有效距离（米）。
        返回: (d_upper, d_lower)，无有效数据时为 (None, None)。
        """
        if self.depth_image is None:
            return None, None
        now = time.time()
        ...
        max_valid = float(self.get_parameter("max_valid_depth_m").value)
        depth_percentile = float(self.get_parameter("depth_percentile").value)
        width_ratio = float(self.get_parameter("vertical_roi_width_ratio").value)
        cx = w // 2
        roi_w = max(1, int(w * width_ratio))
        x0 = max(0, cx - roi_w // 2)
        x1 = min(w, x0 + roi_w)

        # 上 1/3：图像行 0 ~ h/3 → 机体上方
        y_upper_end = max(1, h // 3)
        roi_upper = self.depth_image[0:y_upper_end, x0:x1]
        roi_upper = roi_upper[np.isfinite(roi_upper)]
        roi_upper = roi_upper[(roi_upper >= min_valid) & (roi_upper <= max_valid)]
        d_upper = float(np.percentile(roi_upper, depth_percentile)) if roi_upper.size > 0 else None

        # 下 1/3：图像行 2h/3 ~ h → 机体下方
        y_lower_start = min(h - 1, 2 * (h // 3))
        roi_lower = self.depth_image[y_lower_start:h, x0:x1]
        roi_lower = roi_lower[np.isfinite(roi_lower)]
        roi_lower = roi_lower[(roi_lower >= min_valid) & (roi_lower <= max_valid)]
        d_lower = float(np.percentile(roi_lower, depth_percentile)) if roi_lower.size > 0 else None

        return d_upper, d_lower

    def _global_front_min_depth(self):
        """
        前方视野内全局最近有效距离（米）。用于检测“比摄像头大的障碍物”：
        若整幅视野都很近，说明可能是墙/大物体，应停止前进并爬升。
        返回 None 表示无有效深度数据。
        """
        if self.depth_image is None:
            return None
        ...
        if roi.size == 0:
            return None
        return float(np.percentile(roi, 5.0))

    def _adjust_altitude_for_vertical_obstacles(self, zD_goal: float) -> float:
        """
        根据深度图上下方障碍，在目标高度 zD_goal 基础上做上下偏移（NED：更负=更高）。
        上方近→向下避让（spD 增大）；下方近→向上避让（spD 减小）。
        """
        if not bool(self.get_parameter("vertical_avoid_enable").value):
            return zD_goal
        d_upper, d_lower = self._depth_vertical_bands()
        near_m = float(self.get_parameter("vertical_near_m").value)
        offset_max = float(self.get_parameter("vertical_offset_max_m").value)

        spD = zD_goal
        # 上方有近障（天花板/树冠）→ 向下避让：spD 取更“大”（更向下）
        if d_upper is not None and d_upper < near_m:
            down_step = min(offset_max, (near_m - d_upper) * 0.5)
            spD = max(spD, self.pz + down_step)
        # 下方有近障（地面/矮障）→ 向上避让：spD 取更“小”（更向上）
        if d_lower is not None and d_lower < near_m:
            up_step = min(offset_max, (near_m - d_lower) * 0.5)
            spD = min(spD, self.pz - up_step)
        # 限制在目标高度附近，避免长期偏离
        spD = max(zD_goal - offset_max, min(zD_goal + offset_max, spD))
        return spD
    ...
    def _compute_setpoint_from_goal(self, goalN, goalE, goalD):
        ...
        实现：水平 VFH+ 选向 + 上下避障调高。
        - 水平：深度图扇区 + 速度门控 + 无扇区时 fallback 直线追目标或 yaw 扫描。
        - 上下：深度图分为上/下条带，上方近→向下避让，下方近→向上避让，spD 在目标高度附近偏移。
        """
        # 1) 计算全局“理想”目标航向与俯仰（前方全角度用）
        vN = goalN - self.px
        vE = goalE - self.py
        goal_dist_xy = math.hypot(vN, vE)
        goal_yaw = radians_wrap(math.atan2(vE, vN))
        # 目标相对水平面的俯仰角（弧度）：上正下负，NED 下为正故用 -(goalD - pz)
        goal_elevation = math.atan2(-(goalD - self.pz), goal_dist_xy) if goal_dist_xy > 1e-3 else 0.0

        # 2) 选方向：2D=前方全角度(水平+垂直)，否则仅水平
        use_2d = bool(self.get_parameter("use_2d_sectors").value)
        best_elevation = 0.0
        if use_2d:
            best_yaw, best_dist, best_elevation = self._vfh_choose_direction_2d(goal_yaw, goal_elevation)
        else:
            best_yaw, best_dist = self._vfh_choose_direction(goal_yaw)

        zD_goal = goalD
        # 上下避障：根据深度图上下方障碍微调高度 setpoint
        spD = self._adjust_altitude_for_vertical_obstacles(zD_goal)
        # 前方全局最近距离：用于“比视野大的障碍物”检测
        global_min = self._global_front_min_depth()
        large_thresh = float(self.get_parameter("large_obstacle_threshold_m").value)
        climb_m = float(self.get_parameter("large_obstacle_climb_m").value)

        def apply_large_obstacle_hold_and_climb():
            """前方整块都很近时：停住并爬升，避免撞大墙。"""
            spD_climb = min(spD, self.pz - climb_m)
            self._debug_throttled(
                f"[algo] large_obstacle | global_min={global_min:.2f}m < {large_thresh}m -> hold + climb to spD={spD_climb:.2f}"
            )
            return self.px, self.py, spD_climb, self.heading if best_yaw is None else best_yaw

        # 3) 如果深度不可用/没有任何可行扇区：
        #    - 默认退化为“直线追目标”（先验证会动、能追目标）
        #    - 如关闭 fallback，则保持原来的 yaw 扫描策略
        if best_yaw is None or best_dist is None:
            if global_min is not None and global_min < large_thresh:
                return apply_large_obstacle_hold_and_climb()
        ...
        # 比视野大的障碍物：前方整体最近距离很近时，不信任“某扇区较远”，停住并爬升
        if global_min is not None and global_min < large_thresh:
            return apply_large_obstacle_hold_and_climb()
        ...
        # 5) 沿着选中的方向给出 waypoint（水平 + 可选垂直分量，实现前方全角度）
        dirN = math.cos(best_yaw)
        dirE = math.sin(best_yaw)

        spN = self.px + dirN * step_xy
        spE = self.py + dirE * step_xy
        # spD：上下避障已给出；2D 时再按选中扇区俯仰角微调（上正=略升，下负=略降）
        if use_2d:
            elev_z = -step_xy * math.tan(best_elevation)
            offset_max = float(self.get_parameter("vertical_offset_max_m").value)
            spD = spD + max(-offset_max, min(offset_max, elev_z))
            spD = max(zD_goal - offset_max, min(zD_goal + offset_max, spD))
        spYaw = best_yaw
        self._debug_throttled(
