...
def is_point_free_in_grid(n: float, e: float, d: float, grid, center_n: float, center_e: float, center_d: float,
    ...
    """True if NED point (n,e,d) lies in grid bounds and the cell is free or buffer (not obstacle/unknown)."""
    ijk = world_to_grid_3d(n, e, d, center_n, center_e, center_d, half_n, half_e, half_d,
                           res_xy, res_z, size_n, size_e, size_d)
    if ijk is None:
        return False
    i, j, k = ijk
    return grid[i, j, k] not in (CELL_OBSTACLE, CELL_UNKNOWN)


def mark_ray_free_3d(grid: np.ndarray, i0: int, j0: int, k0: int, i1: int, j1: int, k1: int):
    """
    Mark all voxels along the 3D line from (i0,j0,k0) to (i1,j1,k1) as CELL_FREE.
    Used for 8.3: unobserved = blocked; only cells hit by a depth ray are marked free.
    """
    sn, se, sd = grid.shape
    di, dj, dk = i1 - i0, j1 - j0, k1 - k0
    n_steps = max(abs(di), abs(dj), abs(dk), 1)
    for s in range(n_steps + 1):
        t = s / n_steps if n_steps > 0 else 1.0
        i = int(round(i0 + t * di))
        j = int(round(j0 + t * dj))
        k = int(round(k0 + t * dk))
        if 0 <= i < sn and 0 <= j < se and 0 <= k < sd:
            grid[i, j, k] = CELL_FREE


def mark_center_neighborhood_free(grid: np.ndarray, ci: int, cj: int, ck: int):
    """
    Mark center cell and its 26 neighbors as CELL_FREE so the planner always has a valid start.
    (Drone is at center — we treat immediate vicinity as observed free.)
    """
    sn, se, sd = grid.shape
    for di in (-1, 0, 1):
        for dj in (-1, 0, 1):
...
# Grid cell values: 0=free, 1=obstacle (blocked), 2=buffer (flyable but high cost), 3=unknown (unobserved, traversable with high cost)
CELL_FREE = 0
CELL_OBSTACLE = 1
CELL_BUFFER = 2
CELL_UNKNOWN = 3
...
def astar_3d(grid: np.ndarray, start_ijk: tuple, goal_ijk: tuple, allow_diagonal_3d: bool = True,
             buffer_cost: float = 1.0, unknown_cost: float = 10.0):
    """
    A* on 3D grid. grid[i,j,k]: 0=free, 1=obstacle, 2=buffer (cost buffer_cost), 3=unknown (cost unknown_cost).
    Unknown is traversable but high cost so we prefer observed free space; path can cross unobserved to reach goal.
    """
    sn, se, sd = grid.shape
    si, sj, sk = start_ijk
    gi, gj, gk = goal_ijk
    ...
    while open_heap:
        ...
        for di, dj, dk in neighbors:
            ...
            if cell == CELL_BUFFER:
                step_cost = step * buffer_cost
            elif cell == CELL_UNKNOWN:
                step_cost = step * unknown_cost
            else:
                step_cost = step * 1.0
...
class AStarGridPursuit(Node):
    def __init__(self):
        ...
        self.declare_parameter('unknown_cost', 10.0)  # A* cost for unobserved cells (traversable but prefer observed free)
        ...
        self.declare_parameter('max_angular_vel_for_replan_rad_s', 0.5)  # skip replan when angular velocity magnitude exceeds this (rad/s)
    ...
    def on_angular_velocity(self, msg):
        """Update body angular velocity (rad/s) for replan skip when turning fast."""
    ...
    def build_3d_grid_ned(self, inflation_override=None) -> tuple:
        ...
        # 8.3: unobserved = blocked. Grid starts as unknown; only ray-hit cells become free, endpoint = obstacle.
        grid = np.full((size_n, size_e, size_d), CELL_UNKNOWN, dtype=np.uint8)
        center_n, center_e, center_d = self.px, self.py, self.pz
        center_ijk = (size_n // 2, size_e // 2, size_d // 2)
        fx, fy = self.cam_fx, self.cam_fy
        cx, cy = self.cam_cx, self.cam_cy

        if self.depth_image is None or self.depth_image.size == 0:
            # No depth: assume free space so we can still plan to goal (avoid stuck hover)
            grid.fill(CELL_FREE)
            if self.get_parameter('debug_obstacle').value:
        ...
        if n_valid == 0:
            # All depth at max range / invalid: assume free space so we can plan (e.g. looking at sky)
            grid.fill(CELL_FREE)
            return grid, size_n, size_e, size_d, res_xy_actual, res_z_actual, half_xy, half_xy, half_d
        ...
        if near_count < 10:
            ...
            # No near obstacles (e.g. all at max range): assume free space so we can plan
            grid.fill(CELL_FREE)
            if self.get_parameter('debug_obstacle').value:
                self._last_grid_diag = {
        ...
        gj = gj[in_bounds]
        gk = gk[in_bounds]
        # 8.3: mark ray from camera (center) to each depth point as FREE, then endpoint as OBSTACLE
        ci, cj, ck = center_ijk
        for idx in range(len(gi)):
            mark_ray_free_3d(grid, ci, cj, ck, int(gi[idx]), int(gj[idx]), int(gk[idx]))
        ...
        # Ensure center + 26 neighbors are FREE so planner always has a valid start (drone is there)
        mark_center_neighborhood_free(grid, center_ijk[0], center_ijk[1], center_ijk[2])
    ...
    def plan_path_ned(self) -> list:
        ...
        path_ijk = astar_3d(grid, start_ijk, goal_ijk, allow_diagonal_3d=True, buffer_cost=buffer_cost, unknown_cost=unknown_cost)
        ...
        if reuse and old_path:
            ...
            if valid_old:
                ...
                if last_ijk is not None:
                    tail_ijk = astar_3d(grid, last_ijk, goal_ijk, allow_diagonal_3d=True, buffer_cost=buffer_cost, unknown_cost=unknown_cost)
                    if tail_ijk:
                        tail_ned = []
