...
Topics:
  ...
  sub: /fmu/out/vehicle_angular_velocity    (px4_msgs/VehicleAngularVelocity, optional) - skip replan when |yaw_rate| high
...
try:
    ...
    _HAS_VEHICLE_ANGULAR_VELOCITY = True
except ImportError:
    _HAS_VEHICLE_ANGULAR_VELOCITY = False
...
def world_ned_to_world_grid_3d(n: float, e: float, d: float,
                                center_n: float, center_e: float, center_d: float,
                                half_n: float, half_e: float, half_d: float,
                                res_xy: float, res_z: float,
                                size_n: int, size_e: int, size_d: int):
    """Map world NED (n, e, d) to world 3D grid (wi, wj, wk). Returns (wi, wj, wk) or None if outside."""
    wi = int((n - center_n + half_n) / res_xy)
    wj = int((e - center_e + half_e) / res_xy)
    wk = int((d - center_d + half_d) / res_z)
    if 0 <= wi < size_n and 0 <= wj < size_e and 0 <= wk < size_d:
        return (wi, wj, wk)
    return None
...
class AStarGridPursuit(Node):
    def __init__(self):
        ...
        self.declare_parameter('buffer_cells', 3)     # extra layer around inflation: flyable but high cost (avoids delay overshoot)
        self.declare_parameter('buffer_cost', 30.0)   # A* cost multiplier for buffer cells (prefer paths outside buffer)
        ...
        # World grid (8.4: soft cost only — world grid cells become buffer, not obstacle)
        self.declare_parameter('world_grid_half_size_m', 30.0)   # world grid ±half in N/E (m)
        self.declare_parameter('world_grid_half_height_m', 8.0)   # world grid ±half in D (m)
        self.declare_parameter('max_yaw_rate_for_replan_rad_s', 0.5)  # skip replan when |yaw_rate| > this (rad/s)
        ...
        # World grid 3D (NED, origin at home): used only as soft cost (buffer), never hard obstacle
        self.world_grid_3d = None    # (size_n, size_e, size_d) uint8, 1=occupied
        self._world_grid_attrs = None  # (size_n, size_e, size_d, res_xy, res_z, half_n, half_e, half_d, center_n, center_e, center_d)
        self.angular_vel_z = 0.0     # body yaw rate (rad/s), for skip-replan when turning fast
        ...
        if _HAS_VEHICLE_ANGULAR_VELOCITY:
    ...
    def on_angular_velocity(self, msg):
        """Body angular velocity (rad/s); xyz[2] = yaw rate. Used to skip replan when turning fast."""
        if not _HAS_VEHICLE_ANGULAR_VELOCITY:
            return
        self.angular_vel_z = float(msg.xyz[2])
    ...
    def _ensure_world_grid_3d(self, res_xy: float, res_z: float):
        """Allocate persistent 3D world grid (NED origin at 0,0,0) if not yet created. 8.4: used only as soft cost."""
        if self.world_grid_3d is not None:
            return
        half_xy = float(self.get_parameter('world_grid_half_size_m').value)
        half_d = float(self.get_parameter('world_grid_half_height_m').value)
        size_n = int(2 * half_xy / res_xy + 1)
        size_e = int(2 * half_xy / res_xy + 1)
        size_d = int(2 * half_d / res_z + 1)
        size_n = max(20, min(400, size_n))
        size_e = max(20, min(400, size_e))
        size_d = max(10, min(200, size_d))
        res_xy_w = 2 * half_xy / (size_n - 1) if size_n > 1 else res_xy
        res_z_w = 2 * half_d / (size_d - 1) if size_d > 1 else res_z
        self.world_grid_3d = np.zeros((size_n, size_e, size_d), dtype=np.uint8)
        self._world_grid_attrs = (
            size_n, size_e, size_d, res_xy_w, res_z_w,
            half_xy, half_xy, half_d,
            0.0, 0.0, 0.0,  # center NED = origin
        )
        self.get_logger().info(
            "[world_grid_3d] soft-cost only map ±%.0fm xy ±%.0fm d res_xy=%.3f res_z=%.3f -> %dx%dx%d"
            % (half_xy, half_d, res_xy_w, res_z_w, size_n, size_e, size_d)
        )

    def _update_world_grid_3d_from_ned_points(self, P_ned: np.ndarray):
        """Write current-frame obstacle points (NED) into world grid (accumulate, 1=occupied)."""
        if self.world_grid_3d is None or P_ned is None or P_ned.size == 0:
            return
        (size_n, size_e, size_d, res_xy, res_z, half_n, half_e, half_d,
         wc_n, wc_e, wc_d) = self._world_grid_attrs
        for idx in range(P_ned.shape[0]):
            n, e, d = float(P_ned[idx, 0]), float(P_ned[idx, 1]), float(P_ned[idx, 2])
            ijk = world_ned_to_world_grid_3d(
                n, e, d, wc_n, wc_e, wc_d,
                half_n, half_e, half_d, res_xy, res_z, size_n, size_e, size_d,
            )
            if ijk is not None:
                wi, wj, wk = ijk
                self.world_grid_3d[wi, wj, wk] = 1

    def _overlay_world_grid_3d_as_soft_cost(self, grid: np.ndarray,
                                            size_n: int, size_e: int, size_d: int,
                                            center_n: float, center_e: float, center_d: float,
                                            half_n: float, half_e: float, half_d: float,
                                            res_xy: float, res_z: float):
        """8.4: World grid cells overlay as CELL_BUFFER only (soft cost), never CELL_OBSTACLE. Only touch CELL_FREE."""
        if self.world_grid_3d is None or self._world_grid_attrs is None:
            return
        (wsn, wse, wsd, wres_xy, wres_z, whalf_n, whalf_e, whalf_d,
         wc_n, wc_e, wc_d) = self._world_grid_attrs
        for i in range(size_n):
            for j in range(size_e):
                for k in range(size_d):
                    if grid[i, j, k] != CELL_FREE:
                        continue
                    n = center_n - half_n + (i + 0.5) * res_xy
                    e = center_e - half_e + (j + 0.5) * res_xy
                    d = center_d - half_d + (k + 0.5) * res_z
                    ijk = world_ned_to_world_grid_3d(
                        n, e, d, wc_n, wc_e, wc_d,
                        whalf_n, whalf_e, whalf_d, wres_xy, wres_z, wsn, wse, wsd,
                    )
                    if ijk is not None and self.world_grid_3d[ijk[0], ijk[1], ijk[2]] == 1:
                        grid[i, j, k] = CELL_BUFFER
    ...
    def build_3d_grid_ned(self, inflation_override=None) -> tuple:
        ...
        # World grid 8.4: accumulate current-frame obstacles; overlay later as soft cost only
        self._ensure_world_grid_3d(res_xy_actual, res_z_actual)
        self._update_world_grid_3d_from_ned_points(P_ned[in_bounds])
        ...
        # 8.4: World grid only as soft cost — overlay world occupied as CELL_BUFFER, never CELL_OBSTACLE
        self._overlay_world_grid_3d_as_soft_cost(
            grid, size_n, size_e, size_d,
            center_n, center_e, center_d, half_xy, half_xy, half_d,
            res_xy_actual, res_z_actual,
        )
    ...
    def loop(self):
        ...
        if self.stage == Stage.PLAN_FOLLOW:
            ...
            do_replan = (not self.path_ned or (t_s - self.last_replan_s) >= replan_interval)
            if do_replan and _HAS_VEHICLE_ANGULAR_VELOCITY:
                max_yaw_rad_s = float(self.get_parameter('max_yaw_rate_for_replan_rad_s').value)
                if abs(self.angular_vel_z) > max_yaw_rad_s:
                    do_replan = False  # angle velocity too large: do not replan
            if do_replan:
